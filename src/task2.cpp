...
void place_simplified_meshes_at_points(
    ...
    if(random_rotation){
        std::cout << "Random_rotation: enabled" << std::endl;
    }else{
        std::cout << "Random_rotation: disabled" << std::endl;
    }
    ...
    for (size_t i = 0; i < gaussian_points.size(); ++i) {
        ...
        if(simplification) {
            ...
            SMS::Edge_count_ratio_stop_predicate<Surface_mesh> stop(simplify_ratio);
            SMS::edge_collapse(current_mesh, stop);
        }
...
int main(int argc, char* argv[]) {
    ...
    std::string output_file = "task2.off";
    double model_scale = 0.1;
    int num_points = 500; // 默认值
    //高斯分布参数
    ...
    if(argc >= 3){
        ...
        try{
            num_points = std::stoi(arg2);
            if(num_points <= 0){
                std::cerr << "错误：点的数量必须是正整数" << std::endl;
                return -1;
            }
        } catch(const std::invalid_argument& e){
            std::cerr << "错误：无法解析点的数量，请提供一个有效的整数" << std::endl;
            return -1;
        } catch(const std::out_of_range& e){
            std::cerr << "错误：点的数量超出了有效范围" << std::endl;
            return -1;
        }
    }
    ...
    // 将模型放置到点上
    place_simplified_meshes_at_points(model_points, models_dir, output_file, model_scale, simplification, random_rotation);
