...
// 生成三维齐夫分布的点
vector<Point_3> generate_zipf_points(int num_points, double s, int n) {
    random_device rd;
    mt19937 gen(rd());
    ...
    for (int i = 0; i < num_points; ++i) {
        ...
        x += uniform_real_distribution<double>(-diffusion_factor * rank, diffusion_factor * rank)(gen);
        y += uniform_real_distribution<double>(-diffusion_factor * rank, diffusion_factor * rank)(gen);
        z += uniform_real_distribution<double>(-diffusion_factor * rank, diffusion_factor * rank)(gen);
...
// 获取模型文件列表
vector<string> get_models(const string& models_dir) {
    vector<string> model_files;

    fs::path bed_dir = fs::path(models_dir) / "bed";
    
    if (!fs::exists(bed_dir) || !fs::is_directory(bed_dir)) {
        cerr << "Error: Model directory not found: " << bed_dir << std::endl;
        return model_files;
    }

    for (const auto& entry : fs::directory_iterator(bed_dir)) {
...
// 将模型填充到分布的点上
void fill_points_with_models(
    const vector<Point3D>& model_points,
    ...
    // 获取所有模型文件
    vector<std::string> bed_models = get_models(models_dir);
    if (bed_models.empty()) {
        cerr << "Error: No models found!" << std::endl;
        return;
    }

    cout << "Found " << bed_models.size() << "models." << std::endl;
    cout << "Placing models at " << model_points.size() << " Gaussian points..." << std::endl;
    
    if (random_rotation) {
        cout << "Random rotation: enabled (each model will have random orientation)" << std::endl;
    } else {
        cout << "Random rotation: disabled (models keep original orientation)" << std::endl;
    }

    // 合并后的所有顶点和面
    vector<Point3D> all_vertices;
    vector<vector<int>> all_faces;

    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<size_t> model_dist(0, bed_models.size() - 1);
    
    // 用于生成随机旋转角度的分布（0到2π）
    // 注意：使用全范围旋转，确保每个模型都有不同的朝向
    uniform_real_distribution<double> angle_dist(0.0, 2.0 * M_PI);
    ...
    for (size_t i = 0; i < model_points.size(); ++i) {
        ...
        const string& model_path = bed_models[model_idx];
        ...
        if (random_rotation) {
            ...
            if (i < 5) {
                cout << "Model " << (i + 1) << " rotation angles (degrees): "
                     << "X=" << (angle_x * 180.0 / M_PI) << ", "
                     << "Y=" << (angle_y * 180.0 / M_PI) << ", "
                     << "Z=" << (angle_z * 180.0 / M_PI) << endl;
            }
        ...
        if ((i + 1) % 100 == 0) {
            cout << "Processed " << (i + 1) << " / " << model_points.size() << " points..." << endl;
        }
    }
    ...
    out_file.close();
    cout << "Successfully created " << output_file << " with " 
              << all_vertices.size() << " vertices and " << all_faces.size() << " faces." << endl;
}
...
int main(int argc,char* argv[]){
    ...
    double s = 1.0;
    int n = 1000;
    string models_dir = "/home/dcc/code/MODELS";
    string output_file = "zipf.off";
    double model_scale = 0.1;
    bool random_rotation = true;  // 是否随机旋转模型
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (arg == "-n" || arg == "--num-points") {
                ...
                std::cout << "Usage: " << argv[0] << " [options]\n"
                          ...
                          << "  -m, --s VALUE          Mean of Gaussian distribution (default: 0.0)\n"
                          << "  -s, --n VALUE        Standard deviation of Gaussian distribution (default: 1.0)\n"
    ...
    auto points_cgal = generate_zipf_points(num_points,s,n);
    cout << "Generated " << points_cgal.size() << " points." << endl;
    auto points = convert_points(points_cgal);
    fill_points_with_models(points,models_dir,output_file,model_scale,true);
    return 0;
}